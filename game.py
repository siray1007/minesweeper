"""
扫雷游戏 - 核心模块
包含游戏逻辑 (MinesweeperGame) 和图形界面 (GameWindow)
"""
...
class MinesweeperGame:
    ...
    def generate_mines(self, safe_row: int, safe_col: int):
        """在首次点击后生成地雷，确保首点及其周围安全"""
        safe = {(safe_row, safe_col)}
        # 仅在棋盘足够大时扩展安全区（防止小棋盘安全区覆盖全盘）
        total_cells = self.rows * self.cols
        if total_cells > self.total_mines + 9:
            for dr in range(-1, 2):
                for dc in range(-1, 2):
    ...
    def reveal(self, row: int, col: int) -> str:
        """翻开一格。返回 'continue' / 'game_over' / 'win'"""
    ...
    def chord(self, row: int, col: int) -> str:
        """双击已翻开的数字格：若周围旗数 == 该格数字，自动翻开周围非旗格"""
        if self.game_over or self.game_won:
            return 'continue'
        ...
        if flag_cnt != self.board[row][col]:
            return 'continue'
            ...
            for dc in (-1, 0, 1):
                ...
                if 0 <= nr < self.rows and 0 <= nc < self.cols:
                    if not self.revealed[nr][nc] and not self.flagged[nr][nc]:
                        ...
                        if r == 'game_over':
                            final = 'game_over'
                        elif r == 'win' and final != 'game_over':
                            final = 'win'
        return final
...
class GameWindow(tk.Toplevel):
    ...
    def __init__(self, master, user: dict, difficulty: str, on_close):
        ...
        elif difficulty == '27x27':
            self.geometry("660x710")
        else:
    ...
    def _build_ui(self):
        ...
        self.mine_label = tk.Label(bar, font=('Consolas', 16, 'bold'),
                                   bg='#d0d0d0', fg='#333')
        self.mine_label.pack(side=tk.LEFT, padx=20)

        tk.Label(bar, text=self.cfg['title'], font=('微软雅黑', 13),
                 bg='#d0d0d0', fg='#555').pack(side=tk.LEFT, expand=True)

        self.timer_label = tk.Label(bar, text="⏱ 00:00",
    ...
    def _build_small_board(self):
        w = self.cfg['cols'] * self.cell_size
        h = self.cfg['rows'] * self.cell_size
        self.canvas = tk.Canvas(self, width=w, height=h, bg='#bdbdbd',
                ...
                if g.revealed[r][c]:
                    ...
                    if val == -1:
                        ...
                        self.canvas.create_text(cx, cy, text=str(val),
                                                font=('Arial', cs // 2, 'bold'),
                                                fill=NUM_COLORS.get(val, '#000'))
                elif g.flagged[r][c]:
                    self.canvas.create_rectangle(x1, y1, x2, y2,
    ...
    def _build_large_board(self):
        ...
        cw = min(self.cfg['cols'] * self.cell_size, 780)
        ch = min(self.cfg['rows'] * self.cell_size, 560)

        self.scroll_canvas = tk.Canvas(outer, width=cw, height=ch,
                                       bg='#bdbdbd', highlightthickness=0)
        self.h_bar = ttk.Scrollbar(outer, orient=tk.HORIZONTAL,
                                   command=self.scroll_canvas.xview)
        self.v_bar = ttk.Scrollbar(outer, orient=tk.VERTICAL,
                                   command=self.scroll_canvas.yview)
        self.scroll_canvas.configure(xscrollcommand=self.h_bar.set,
        ...
        fw = self.cfg['cols'] * self.cell_size
        fh = self.cfg['rows'] * self.cell_size

        self.canvas = tk.Canvas(self.scroll_canvas, width=fw, height=fh,
                                bg='#bdbdbd', highlightthickness=0)
        ...
        # 缩放控件
        zf = tk.Frame(self, bg='#e0e0e0')
        zf.pack(fill=tk.X, padx=10, pady=(0, 5))
    ...
    def _draw_large(self):
        """绘制大板：线条 + 增量覆盖"""
        self.canvas.delete('all')
        cs = max(2, int(self.cell_size * self.zoom))
        self._actual_cs = cs
        g = self.game
        ...
        for r in range(g.rows + 1):
            y = r * cs
            self.canvas.create_line(0, y, g.cols * cs, y, fill='#aaa', width=1)
        for c in range(g.cols + 1):
            x = c * cs
            self.canvas.create_line(x, 0, x, g.rows * cs, fill='#aaa', width=1)
            ...
            for c in range(g.cols):
                ...
                if g.revealed[r][c]:
                    ...
                    if val == -1:
                        if cs >= 10:
                            self.canvas.create_text(cx, cy, text='💣',
                                                    font=('Arial', max(8, cs // 2)))
                    elif val > 0:
                        if cs >= 10:
                            self.canvas.create_text(cx, cy, text=str(val),
                                                    font=('Arial', max(8, cs // 2), 'bold'),
                                                    fill=NUM_COLORS.get(val, '#000'))
                        else:
                            # 太小时用色块表示
                            clr = {1: '#bbdefb', 2: '#c8e6c9', 3: '#ffcdd2',
                                   4: '#b39ddb', 5: '#ffccbc', 6: '#b2dfdb',
                                   7: '#cfd8dc', 8: '#d7ccc8'}.get(val, '#d8d8d8')
    ...
    def _update_mine_label(self):
        self.mine_label.config(text=f"💣 {self.game.remaining_mines}")
