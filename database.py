...
if getattr(sys,'frozen',False):_APP_DIR=os.path.dirname(os.path.abspath(sys.executable))
else:_APP_DIR=os.path.dirname(os.path.abspath(__file__))
DB_PATH=os.path.join(_APP_DIR,'minesweeper.db')

GITEE_USER='siray-07';GITEE_REPO='minesweeper'
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS rankings(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,difficulty TEXT NOT NULL,time_seconds INTEGER NOT NULL,completed_at TEXT NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id))''')
...
def register_user(username:str,password:str)->tuple:
    ...
    if not username or not password:return False,t('warn_empty')
    if len(username)<2:return False,t('user_short')
    if len(password)<4:return False,t('pwd_short')
    conn=get_db();c=conn.cursor()
    try:
        pwd_hash=hash_password(password)
        c.execute('INSERT INTO users(username,password_hash,created_at) VALUES(?,?,?)',(username,pwd_hash,datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        conn.commit();return True,t('reg_ok')
    except sqlite3.IntegrityError:return False,t('user_exists')
    finally:conn.close()

def login_user(username:str,password:str)->tuple:
    conn=get_db();c=conn.cursor();pwd_hash=hash_password(password)
    c.execute('SELECT id,username FROM users WHERE username=? AND password_hash=?',(username.strip(),pwd_hash))
...
def save_ranking(user_id:int,difficulty:str,time_seconds:int,username:str=''):
    conn=get_db();c=conn.cursor()
    c.execute('INSERT INTO rankings(user_id,difficulty,time_seconds,completed_at) VALUES(?,?,?,?)',(user_id,difficulty,time_seconds,datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    conn.commit();conn.close()
    if username:_gitee_append_ranking(username,difficulty,time_seconds)
...
def _gitee_fetch_rankings(difficulty:str,limit:int)->list|None:
    try:
        ...
        with urllib.request.urlopen(req,timeout=8) as resp:all_rankings=json.loads(resp.read().decode())
...
def _gitee_append_ranking(username:str,difficulty:str,time_seconds:int):
    try:
        ...
        with urllib.request.urlopen(req,timeout=10) as resp:info=json.loads(resp.read().decode())
        sha=info['sha'];content=base64.b64decode(info['content']).decode()
        rankings=json.loads(content) if content.strip() else []
    except Exception:return
    rankings.append({'username':username,'difficulty':difficulty,'time_seconds':time_seconds,'created_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S')})
    best={}
    for r in rankings:
        key=(r['username'],r['difficulty'])
        if key not in best or r['time_seconds']<best[key]['time_seconds']:best[key]=r
    ...
    body=json.dumps({'access_token':GITEE_TOKEN,'content':base64.b64encode(new_content.encode()).decode(),'sha':sha,'message':f'📊 {username} {difficulty} {time_seconds}s'}).encode()
    try:
        put_req=urllib.request.Request(GITEE_API,data=body,method='PUT',headers={'Content-Type':'application/json'})
