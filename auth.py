...
_BG = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bg_gradient_login.png')

def _bomb_label(parent, text, font, **kw):
    f = tk.Frame(parent, bg=kw.get('bg', '#f0f5ff'))
    if os.path.exists(_ICON):
        img = tk.PhotoImage(file=_ICON); f.img = img
        tk.Label(f, image=img, bg=kw.get('bg', '#f0f5ff')).pack(side=tk.LEFT, padx=(0,6))
    tk.Label(f, text=text, font=font, bg=kw.get('bg', '#f0f5ff'), fg=kw.get('fg','#333')).pack(side=tk.LEFT)
    return f

class AuthFrame(tk.Frame):
    def __init__(self, parent, on_login):
        super().__init__(parent, bg='#f0f5ff')
        self.on_login = on_login
        if os.path.exists(_BG):
            self._bg = tk.PhotoImage(file=_BG)
            tk.Label(self, image=self._bg, bg='#f0f5ff').place(x=0,y=0,relwidth=1,relheight=1)
        self._show_login()
    def _clear(self):
        for w in self.winfo_children(): w.destroy()
    def _lang_selector(self):
        f = tk.Frame(self, bg='#f0f5ff'); f.place(x=10, y=10)
        names = [o[1] for o in LANG_OPTIONS]
        cur_name = dict(LANG_OPTIONS).get(get_lang(), names[0])
        var = tk.StringVar(value=cur_name)
        cb = ttk.Combobox(f, textvariable=var, values=names, state='readonly', width=12, font=('微软雅黑', 9))
        cb.pack()
    ...
    def _show_login(self):
        ...
        _bomb_label(self, t('title'), ('微软雅黑', 26, 'bold'), bg='#f0f5ff', fg='#333').pack(pady=(50,5))
        tk.Label(self, text=t('login'), font=('微软雅黑', 14), bg='#f0f5ff', fg='#666').pack(pady=(0,25))
        f = tk.Frame(self, bg='#f0f5ff'); f.pack()
        tk.Label(f, text=t('username'), bg='#f0f5ff', font=('微软雅黑', 10)).pack()
        self._user = ttk.Entry(f, width=28, font=('微软雅黑', 11)); self._user.pack(pady=(5,10)); self._user.focus_set()
        tk.Label(f, text=t('password'), bg='#f0f5ff', font=('微软雅黑', 10)).pack()
        ...
        link = tk.Frame(f, bg='#f0f5ff'); link.pack()
        tk.Label(link, text=t('no_account'), bg='#f0f5ff', font=('微软雅黑', 9)).pack(side=tk.LEFT)
    ...
    def _show_register(self):
        self._clear(); self._lang_selector()
        _bomb_label(self, t('title'), ('微软雅黑', 26, 'bold'), bg='#f0f5ff', fg='#333').pack(pady=(50,5))
        tk.Label(self, text=t('register'), font=('微软雅黑', 14), bg='#f0f5ff', fg='#666').pack(pady=(0,25))
        f = tk.Frame(self, bg='#f0f5ff'); f.pack()
        tk.Label(f, text=t('username'), bg='#f0f5ff', font=('微软雅黑', 10)).pack()
        self._reg_user = ttk.Entry(f, width=28, font=('微软雅黑', 11)); self._reg_user.pack(pady=(5,10)); self._reg_user.focus_set()
        tk.Label(f, text=t('pwd_hint'), bg='#f0f5ff', font=('微软雅黑', 10)).pack()
        self._reg_pwd = ttk.Entry(f, width=28, font=('微软雅黑', 11), show='●'); self._reg_pwd.pack(pady=(5,10))
        tk.Label(f, text=t('confirm_pwd'), bg='#f0f5ff', font=('微软雅黑', 10)).pack()
        ...
        link = tk.Frame(f, bg='#f0f5ff'); link.pack()
        tk.Label(link, text=t('has_account'), bg='#f0f5ff', font=('微软雅黑', 9)).pack(side=tk.LEFT)
