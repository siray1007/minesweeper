...
class MainApp:
    ...
    def _show_menu(self):
        ...
        tk.Label(self.menu, text=f"欢迎，{self.current_user['username']}！",
        ...
        row = tk.Frame(self.menu, bg='#f0f0f0')
        row.pack(pady=5)

        difficulties = [
            ('9x9',   '🟢 简单 9×9',   '#4CAF50', '10 个地雷 · 经典入门'),
            ('27x27', '🟡 进阶 27×27', '#FF9800', '100 个地雷 · 高手进阶'),
            ('81x81', '🔴 困难 81×81', '#F44336', '800 个地雷 · 极限挑战'),
        ]
        ...
        for diff_key, label, color, desc in difficulties:
            ...
            tk.Label(card, text=label, font=('微软雅黑', 14, 'bold'),
                     bg='white', fg=color).pack(pady=(10, 2))
            tk.Label(card, text=desc, font=('微软雅黑', 9),
                     bg='white', fg='#999').pack(pady=(0, 8))

            btn = tk.Button(card, text="开始游戏", font=('微软雅黑', 10, 'bold'),
        ...
        ttk.Button(bottom, text="🏆 排行榜", command=self._show_ranking,
    ...
    def _show_ranking(self):
        RankingWindow(self.menu, self.current_user)
